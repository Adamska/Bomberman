...
void BombSup::initialize(void)
{
  this->texture_ = gdl::Image::load("Textures/bomb.png");
}

void BombSup::initialize(float const & x, float const & y, float const & z)
{
  this->pos_.setPos(x, y, z);
}

void BombSup::update(gdl::GameClock const & gameClock, gdl::Input & input)
{
  (void) gameClock;
  (void) input;
}

void BombSup::draw(void)
{
  texture_.bind();
  glPushMatrix();
